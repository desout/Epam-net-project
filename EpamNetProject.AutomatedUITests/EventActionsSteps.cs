...
{
    [Binding]
    public class EventActionsSteps
    {
        ...
        public void GivenEventWithNameExistsOnPageOrCreatedNewOne(string name)
        {
            ...
            if(items.Count == 0)
            {
                ...
                ThenFillAllInputOnEditEventPageWithDataName_Description_TimeDifferenceBeetwenTodayAndTime_Title_("Automation test event name", "Automation test event description",100, "Automation test event title");
        ...
        [Then(@"fill all Input on edit event page with data: Name - ""(.*)"", Description - ""(.*)"", Time\(difference beetwen today and time\) - ""(.*)"", Title- ""(.*)""")]
        public void ThenFillAllInputOnEditEventPageWithDataName_Description_TimeDifferenceBeetwenTodayAndTime_Title_(string name, string description, int time, string title)
        {
        ...
        public void ThenIPressButtonInBlockWithClassnameNearParagrahWithClassnameAndText(string blockClassname, string paragraphClassname, string text)
        {
            _driver.FindElement(By.XPath($"//*[@class='{paragraphClassname}' and text()='{text}']/following-sibling::div[@class='{blockClassname}']")).FindElement(By.TagName("button")).Click();
        }
        
        [Then(@"Event with Name ""(.*)"" not exists")]
        public void ThenEventWithNameNotExists(string name)
        {
            Assert.AreEqual(0,_driver.FindElements(By.XPath($"//*[@class='event--item__name' and text()='{name}']")).Count);
        }

        [Then(@"I press link in block with classname ""(.*)"" near paragrah with classname '(.*)' and text ""(.*)""")]
        public void ThenIPressLinkInBlockWithClassnameNearParagrahWithClassnameAndText(string blockClassname, string paragraphClassname, string text)
        {
            _driver.FindElement(By.XPath($"//*[@class='{paragraphClassname}' and text()='{text}']/following-sibling::div[@class='{blockClassname}']")).FindElement(By.TagName("a")).Click();
        }
